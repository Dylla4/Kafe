...
<head>
    ...
    <style>
        ...
        .form-input-focus { @apply focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all; }
...
<body class="bg-stone-50 text-stone-800 p-4 md:p-12">
                        ...
                        <tbody class="divide-y divide-stone-50">
                            ...
                            @foreach($cartItems as $id => $details)
                                ...
                                <tr class="group">
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div class="w-16 h-16 shrink-0 bg-stone-100 rounded-2xl overflow-hidden border border-stone-100 group-hover:scale-105 transition-transform">
                                                @if(isset($details['foto']) && $details['foto'])
                                                    <img src="{{ asset($menu->foto) }}" ... > class="w-full h-full object-cover">
                                                @else
                                    ...
                                    <td class="text-center font-bold text-stone-700 text-sm">
                                        {{ $details['quantity'] }}x
                                    </td>
                ...
                <form id="form-pembayaran" class="bg-orange-50/50 p-8 rounded-3xl border border-orange-100 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="jenis_pesanan" id="pilihan_tipe" class="w-full p-4 border border-stone-200 rounded-2xl text-sm bg-white shadow-sm outline-none focus:ring-2 focus:ring-orange-500 font-bold cursor-pointer">
                        ...
                        <div id="container_meja">
                            ...
                            <input type="text" 
                                    name="nomor_meja" 
                                    value="{{ $mejaOtomatis ?? '01' }}" 
                                    autocomplete="off" {{-- TAMBAHKAN INI --}}
                                    readonly
                                    class="w-full p-4 border border-stone-200 rounded-2xl bg-white font-black text-orange-700 text-center text-lg shadow-sm focus:ring-2 focus:ring-orange-500 outline-none">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="metode_pembayaran" class="w-full p-4 border border-stone-200 rounded-2xl text-sm bg-white shadow-sm font-black text-stone-700">
                            ...
                            <option value="midtrans">💳 Pembayaran Digital (QRIS/Transfer)</option>
                        </select>
                    </div>

                    <div class="pt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <button type="submit" id="btn-submit" class="md:col-span-2 w-full py-5 bg-orange-700 text-white rounded-2xl font-black uppercase tracking-[0.2em] hover:bg-orange-800 shadow-xl transition-all active:scale-[0.98] disabled:bg-stone-300">
                            Konfirmasi Pesanan Sekarang
                        </button>
                        <a href="{{ route('menu') }}" class="md:col-span-2 text-center text-xs font-bold text-stone-400 uppercase tracking-widest hover:text-orange-700 transition-colors py-2">
                            ← Kembali Tambah Menu
                        </a>
                    </div>
                </form>
    ...
    <script>
        ...
        pilihanTipe?.addEventListener('change', function() {
            const isTakeAway = this.value === 'take_away';
            kolomAlamat.classList.toggle('hidden', !isTakeAway);
            containerMeja.classList.toggle('hidden', isTakeAway);
        });

        // Logika Filter Jam Operasional Otomatis
        ...
        document.getElementById('form-pembayaran')?.addEventListener('submit', function(e) {
            ...
            .then(res => res.json())
            .then(data => {
                if (data.success) window.location.href = "/payment/" + data.order_id;
                else { alert(data.error || "Gagal!"); btn.disabled = false; btn.innerHTML = "Konfirmasi Pesanan Sekarang"; }
            })
            .catch(() => { alert("Sistem sibuk."); btn.disabled = false; });
        });
    </script>
</body>
</html>